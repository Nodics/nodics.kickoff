...
function setup(role) {
  ...
  global.CONFIG = { get: key => key === 'data' ? { dataReleases: { allowedContractVersions: [2], maximumFilesPerRelease: 50, maximumModulesPerRun: 10, allowDowngrade: false, destinationEnforced: true, allowedDestinationRoles: [destination], environmentClass: 'LOCAL', types: { sample: { enabled: true, operatorExecution: true } } } } : key === 'defaultTenant' ? 'default' : key === 'runtimeRole' ? { code: destination, publication: 'STAGED' } : undefined };
  global.NODICS = { getActiveModules: () => modules, getRawModule: name => roots[name] ? { name, path: roots[name], parent: `agora.${name.replace(/^agora|Data$/g, '').toLowerCase()}`, canonicalIdentity: name, metaData: { nodics: { displayName: name } } } : undefined, getSelectedEnvironmentName: () => 'kickoffLocal' };

...
function setup(role) {
  ...
  global.CONFIG = { get: key => key === 'environment' ? { class: 'LOCAL' } : key === 'data' ? { dataReleases: { allowedContractVersions: [2], maximumFilesPerRelease: 256, maximumModulesPerRun: 10, allowDowngrade: false, destinationEnforced: true, allowedDestinationRoles: [destination], types: { sample: { enabled: true, operatorExecution: true } } } } : key === 'defaultTenant' ? 'default' : key === 'runtimeRole' ? { code: destination, publication: 'STAGED' } : undefined };

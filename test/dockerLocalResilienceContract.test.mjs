...
const projectCommandService = require(
  path.join(
    frameworkRoot,
    "nodics.foundation/modules/nTooling/src/service/command/defaultProjectCommandService",
  ),
);
...
assert.equal(
  ...
  "--confirm-replace-docker-local-data",
);
assert.equal(
  dockerLocalProfile.resilience.volumes.mediaStaged,
  "nodics-kickoff-docker-local-media-staged",
);
assert.equal(
...
assert.deepEqual(projectCommands["docker-local:backup"].args, [
  "dockerLocal",
  "backup",
]);
assert.deepEqual(projectCommands["docker-local:verify"].args, [
  "dockerLocal",
  "verify",
]);
assert.deepEqual(projectCommands["docker-local:restore"].args, [
  "dockerLocal",
  "restore",
]);
assert.match(
...
assert.deepEqual(projectCommands["docker-local:resilience"].args, [
  "dockerLocal",
...
assert.deepEqual(projectCommands["docker-local:soak"].args, [
  "dockerLocal",
  "soak",
]);
assert.equal(
...
assert.deepEqual(
  projectCommands["acceptance:documentation:fresh-browser"].args,
  ["dockerLocal", "acceptance", "--expect-documentation-not-installed"],
);

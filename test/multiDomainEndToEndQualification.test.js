...
test('Apparel search projection data supports size and colour selection through cart and order composition', () => {
  const products = values(load(path.join(apparelRoot, 'agoraApparelProductData'))); const localizations = values(load(path.join(apparelRoot, 'agoraApparelProductLocalizationData'))); const variants = values(load(path.join(apparelRoot, 'agoraApparelProductVariantData'))); const variantLocalizations = values(load(path.join(apparelRoot, 'agoraApparelProductVariantLocalizationData'))); const styles = values(load(path.join(apparelRoot, 'agoraApparelStyleData'))); const prices = values(load(path.join(apparelRoot, 'agoraApparelPriceRowData'))); const inventory = values(load(path.join(apparelRoot, 'agoraApparelInventoryBalanceData')));
  assert.equal(products.length, 58); assert.equal(localizations.length, 116); assert.equal(variants.length, 464); assert.equal(variantLocalizations.length, 928);
  assert.equal(styles.length, products.length); assert.equal(prices.length, products.length); assert.equal(inventory.length, variants.length); assert.equal(new Set(localizations.map(item => item.locale)).size, 2); assert.equal(prices.every(item => item.currency === 'USD'), true);
  const productCodes = new Set(products.map(item => item.code)); const localizedProductCodes = new Set(localizations.map(item => item.productCode)); const variantProductCodes = new Set(variants.map(item => item.productCode)); const styleProductCodes = new Set(styles.map(item => item.productCode)); const pricedProductCodes = new Set(prices.map(item => item.productCode));
  assert.deepEqual([...styleProductCodes].filter(code => !productCodes.has(code)), []); assert.deepEqual([...productCodes].filter(code => !localizedProductCodes.has(code)), []); assert.deepEqual([...productCodes].filter(code => !variantProductCodes.has(code)), []); assert.deepEqual([...productCodes].filter(code => !pricedProductCodes.has(code)), []);
  ['agoraLinenWrapDress', 'agoraSatinMidiDress', 'agoraRibbedKnitTop', 'agoraLeatherTote', 'agoraOxfordShirt'].forEach(code => assert(productCodes.has(code)));
  const selected = variants.find(item => item.productCode === 'agoraLinenWrapDress') || variants[0]; const style = styles.find(item => item.productCode === selected.productCode); assert.equal(apparelValidation.validateStyle(style).valid, true); assert(selected.attributes.sizeCode); assert(selected.attributes.colorCode);

...
test('domain content is editable in Staged and preserves logical renderer identity for Online projection', () => {
  for (const [domain, title] of [['apparel', 'Apparel'], ['electronics', 'Electronics'], ['telco', 'Telco']]) { const source = values(load(path.join(dataRoot(domain, title), `agora${title}PageData`)))[0]; const staged = { ...source, name: `${source.name} Updated`, publicationStatus: 'STAGED' }; const online = { ...staged, publicationStatus: 'ONLINE' }; assert.equal(online.renderer, `agora.${domain}.page.home`); assert.match(online.name, /Updated$/); }
});

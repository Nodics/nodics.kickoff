...
assert.equal(selectRuntimes(true).length, 13);
...
assert.deepEqual(backendRuntimes.find(runtime => runtime.code === 'waste')?.dependsOn, ['platform', 'location']);
assert.equal(backendRuntimes.find(runtime => runtime.code === 'platform')?.readinessChecks[0]?.path, '/nodics/backoffice/v0/bootstrap/public');

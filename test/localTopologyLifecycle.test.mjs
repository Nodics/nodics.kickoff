...
assert.deepEqual(backendRuntimes.map(runtime => runtime.port), [4300, 4314, 4330, 4312, 4340, 4360, 4352, 4350]);
assert.equal(new Set(backendRuntimes.map(runtime => runtime.port)).size, backendRuntimes.length);
assert.equal(selectRuntimes(false).length, 8);
assert.equal(selectRuntimes(true).length, 11);
